...
Table III and Fig. 2 report RMSE for the baseline, four ABSA variants, and a no-sentiment floor (DeepMF+CBF only, sentiment stream removed entirely from fusion). The floor confirms sentiment is substantially useful in every domain — even the coarse global-score baseline reduces RMSE by 34.0–44.7% relative to the floor — but naive aggregation degrades accuracy relative to that already-useful baseline, and the effect of the two feature-preserving variants (Concat, Concat + confidence) beyond the baseline is domain-dependent rather than uniform.
...
|---|---|---|---|
| No-sentiment floor (DeepMF+CBF only) | 1.2719 ± 0.1228 | 1.3022 ± 0.0526 | 0.9475 ± 0.0156 |
...
\*\*\* Fisher-combined p < 0.001. \* Fisher-combined p < 0.05 (hotel Concat + confidence: p = 0.010 — significant but in the *unfavorable* direction, i.e. worse than baseline; n.s. = not significant, p ≥ 0.05). Bold denotes the best-performing (lowest RMSE) variant per domain; hotel has no variant significantly better than baseline. The no-sentiment floor is reported descriptively (no paired significance test — it is a different feature set, not a sentiment-representation variant) and confirms that *some* sentiment signal (even the coarse global baseline) contributes substantially in every domain: a 34.0–44.7% RMSE reduction from floor to baseline (restaurant 44.5%, e-commerce 44.7%, hotel 34.0%).
...
**Why is Concat + confidence's benefit domain-dependent, and why hotel specifically?** Concatenation and confidence-as-feature avoid the aggregation failure modes above by preserving the full per-aspect vector and letting the downstream DecisionTreeRegressor determine how to use it, rather than pre-committing to an aggregation rule — this explains why Concat and Concat + confidence never *degrade* accuracy the way aggregation does, but not why their improvement over baseline vanishes specifically in hotel. We can rule out aspect coverage (Section IV-C: hotel has the highest coverage of the three domains) and rule out the e-commerce-style variance artifact (hotel's per-seed values are uniformly tight, not a single-outlier effect, Section IV-B). The no-sentiment floor (Table III) points to a more precise explanation than a general DeepMF+CBF ceiling effect: hotel has the *smallest* floor-to-baseline gap of the three domains (34.0% RMSE reduction vs. 44.5–44.7% in restaurant/e-commerce), meaning a single coarse global sentiment score already captures proportionally more of the sentiment-attributable signal available in hotel, leaving the least headroom for aspect-level refinement to add anything further — consistent with, though not proof of, why hotel is the one domain where granularity beyond the global score stops mattering. Distinguishing this from other hotel-specific factors (its smaller item catalog, 2,056 items vs. 3,757–9,226 in the other domains, or its native rather than manually curated aspect taxonomy) is not possible from three domains alone.
...
**Limitations.** The restaurant and e-commerce aspect lexicons (Table II) were manually curated rather than empirically validated, unlike the hotel domain's native taxonomy, likely contributing to e-commerce's lower coverage (45.1%); this does not explain the hotel result (Section IV-C). E-commerce's high seed-to-seed RMSE variance (Table III) is driven by a single train/validation/test split rather than the sentiment-fusion strategy (Section IV-B), which limits how precisely any e-commerce effect size can be estimated with 5 seeds; a design with more seeds or an explicit multi-split protocol would tighten this. An earlier version of the no-sentiment floor ablation (constant-zero sentiment column forced through `NMF(n_components=3)`) produced RMSE 2–3× worse than even the weakest sentiment-fusion variant and non-reproducible values across seeds (two different seeds yielding bit-identical RMSE); this was traced to NMF degenerating on a rank-deficient input and fixed by dropping the sentiment column entirely (`n_components` auto-capping to 2) rather than substituting a constant — the corrected floor (Table III) is stable across seeds and reported here. The ranking metrics use a candidate-set-limited protocol producing near-ceiling values with limited discriminative power; this study's claims rest on RMSE/MAE. Content-based clustering falls back to text/popularity features only for e-commerce and hotel, which lack category metadata — we did not isolate this stream's marginal contribution. Seeds vary only stochastic components downstream of a fixed split and BERT checkpoint, not split-choice variance itself.
