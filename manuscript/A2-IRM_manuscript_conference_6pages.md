...
*Draft prepared for internal review — author order, affiliations, and funding acknowledgment (UM Dana Internal Penelitian Desentralisasi 2026) to be confirmed before submission. This is the condensed 6-page conference version, corresponding to `A2-IRM_IEEE_conference_v2.docx` (the authoritative submission-track document — content here should be transferred there once finalized). See `A2-IRM_manuscript_draft.md` for the full-length version with the complete stability/variance deep-dive (Table IV, Fig. 4) and extended Discussion. Results/Discussion (Sections IV, V) below still report pre-AdamW-fix numbers and are pending the full 90-run rerun (`scripts/rerun_full_matrix_adamw.sh`); Abstract quantitative claims have been replaced with explicit placeholders until then.*
...
Table III and Fig. 2 report RMSE for the baseline and four ABSA variants. The pattern is consistent in direction across all three domains despite substantially different rating distributions, sparsity, and aspect coverage.
