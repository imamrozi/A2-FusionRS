...
*Draft prepared for internal review — author order, affiliations, and funding acknowledgment (UM Dana Internal Penelitian Desentralisasi 2026) to be confirmed before submission. This is the full-length version; `A2-IRM_manuscript_conference_6pages.md` is the condensed 6-page counterpart corresponding to `A2-IRM_IEEE_conference_v2.docx` (the authoritative submission-track document). Results/Discussion (Sections IV, V) below still report pre-AdamW-fix numbers and are pending the full 90-run rerun (`scripts/rerun_full_matrix_adamw.sh`); Abstract quantitative claims have been replaced with explicit placeholders until then.*

---

## Abstract

Hybrid recommender systems that integrate collaborative filtering, content-based filtering, and sentiment analysis have consistently outperformed single-technique baselines, but the sentiment signal in most such systems remains a single global polarity score per review — a representation that discards exactly the aspect-level nuance (food versus service, price versus durability) that makes review text informative in the first place. We adapt the hybrid architecture of Darraz et al. (deep matrix factorization, K-means/agglomerative content-based clustering, and NMF–DecisionTreeRegressor feature fusion) as our baseline, routing sentiment directly to the fusion stage rather than also injecting it into the content-based stream, and replace its global BERT sentiment score with an aspect-based sentiment analysis (ABSA) representation. We evaluate four alternative fusion strategies for injecting ABSA output into the pipeline — naive mean aggregation across matched aspects, confidence-weighted mean aggregation, raw per-aspect score concatenation, and per-aspect score concatenation augmented with explicit per-aspect confidence features — against the global-sentiment baseline and a no-sentiment floor. All configurations are evaluated on three structurally distinct domains — Yelp restaurant reviews, Amazon Electronics reviews, and TripAdvisor hotel reviews — under an identical protocol (5 seeds, paired Wilcoxon significance testing per seed with Fisher-combined p-values). *[Quantitative RMSE/MAE results and significance outcomes below are placeholders pending completion of the full experimental rerun under the corrected DeepMF training configuration (Section III-B); to be finalized before submission.]* Naive aggregation (mean, confidence-weighted mean) is expected to degrade RMSE relative to the baseline (placeholder: XX.X% relative increase, N/5 seeds significant); raw per-aspect concatenation is expected to restore near-baseline parity; concatenation augmented with explicit per-aspect confidence is hypothesized to be the only variant that significantly improves on the baseline (placeholder: X.X% RMSE reduction, N/5 seeds significant), with a placeholder reduction in run-to-run variance. We term this validated tri-modal representation A2-IRM (Aspect-Aware Integrated Representation Model) and position it as the empirical foundation for a subsequent attention-gated fusion architecture (A2-FusionRS).
...
1. We adapt the Darraz et al. hybrid baseline — routing sentiment directly to the fusion stage rather than also through the content-based stream (Section III-B) — end-to-end, and extend the original two-domain (restaurant, hotel) evaluation to a third, structurally distinct domain (e-commerce electronics reviews), generalizing the data loading, preprocessing, and evaluation pipeline to be domain-agnostic rather than Yelp-specific.
2. We design and evaluate four aspect-based sentiment fusion strategies against the global-sentiment baseline under an identical, statistically rigorous protocol (5 seeds, paired Wilcoxon tests, Fisher-combined p-values), testing whether per-aspect score concatenation with confidence as an explicit auxiliary feature — rather than an aggregation weight — is the variant that most consistently improves on the baseline.
3. We test whether this result replicates across three domains with markedly different review structure, rating distributions, and aspect-keyword coverage (45.1–95.9%), which would be evidence for a domain-general mechanism rather than a dataset-specific artifact, and quantify how aspect coverage relates to the observed effect size.
...
We adapt the hybrid architecture of Darraz et al. [6] as our baseline rather than reimplementing it unmodified. Darraz et al.'s design is a recent, well-performing instance of the feature-combination hybrid family [16], [17] (Section II) — combining collaborative filtering, content-based filtering, and BERT sentiment through NMF and a DecisionTreeRegressor — which makes it an appropriate foundation for isolating the sentiment-representation question this paper investigates (Section I). We retain this three-stream structure and static fusion stage but make one architectural adjustment: sentiment reaches the fusion stage exclusively through its own dedicated stream, rather than being additionally injected into the content-based filtering stream as in the original design (Fig. 1). The rationale is twofold. First, the premise motivating sentiment's inclusion in hybrid recommenders in general — that review sentiment is a source of information not already captured by rating and content signals [7], [13] (Section II) — argues for keeping it a single, distinct stream rather than distributing it across two, which risks the fusion regressor receiving a partially redundant signal from two of its three inputs rather than three genuinely independent ones. Second, this simplification is empirically costless in our setting: including aggregated per-item sentiment among the CBF features, tested as an ablation, changed test RMSE by less than 1% in 13 of 15 domain–seed combinations, indicating that the original design's inclusion of sentiment inside CBF contributes negligible signal beyond what the dedicated sentiment stream already supplies.

![Fig. 1. Overview of the adapted architecture. Three raw inputs (rating matrix, item content/metadata, review text) are encoded by three modality-specific streams — DeepMF, content-based clustering, and BERT-based ABSA (sentence split, aspect matching, per-aspect BERT scoring, and confidence weighting; detailed in Section III-C) — concatenated as raw features, reduced via NMF, and passed to a DecisionTreeRegressor to produce the final predicted rating. Sentiment/ABSA output reaches fusion directly, bypassing both the collaborative-filtering and content-based streams.](figures/fig0_architecture_overview.png)

**Collaborative filtering (DeepMF).** A deep matrix factorization model with a 128-dimensional embedding layer for users and items, an element-wise interaction layer, and a feed-forward stack of [256, 128, 64, 32] hidden units with ReLU activation and 0.3 dropout, trained with mean-squared-error loss via the AdamW optimizer (learning rate 0.002, weight decay 0). An earlier configuration using plain SGD (learning rate 0.001, matching the hyperparameters reported by Darraz et al.) was found to converge deterministically to a near-constant predictor across all five evaluation seeds — embedding-level representations collapsed to negligible variance on both training and held-out rows despite an apparently well-behaved, monotonically decreasing training loss, a failure mode a loss curve alone does not reveal — and was replaced with AdamW after confirming the collapse was independent of the out-of-fold and leave-one-out mechanisms described below. No negative sampling is applied; an earlier configuration with a 1:4 negative-to-positive sampling ratio was found empirically to overfit within one to two epochs (validation RMSE degrading monotonically from epoch 2 onward), whereas training without negative sampling converges monotonically over 20 epochs, and this setting is held fixed across all three domains for comparability.

Train-split DeepMF predictions used downstream by the fusion stage are computed out-of-fold — five folds, a freshly initialized model trained per fold, validated for early stopping against the held-out validation split — rather than by querying the fully-trained model on rows it was itself trained on, to avoid the in-sample optimism characteristic of stacked generalization [24]. The fully-trained model (trained on the complete training split) is reserved for genuinely held-out validation and test predictions.

**Content-based filtering (clustering).** Item feature vectors are constructed from up to three sources — one-hot encoded item category (where available; zero-dimensional and automatically dropped for Amazon Electronics and TripAdvisor, which carry no native category metadata), TF-IDF-weighted review text (500 terms), and popularity metrics (review count and mean rating, computed from the training split only to avoid leakage) — concatenated and reduced to 50 principal components. Following Darraz et al., K-means clustering (elbow-selected K) is used for the restaurant and e-commerce domains, and agglomerative clustering for the hotel domain, matching the domain-specific clustering choice reported in the original paper for its own restaurant/hotel split. A user's cluster preference distribution is estimated from training-set interactions, and item scores are predicted via cosine similarity between an item's cluster assignment and the user's preference distribution.

For training-split rows, the item profile used to determine cluster assignment is recomputed excluding that row's own review before scoring it (leave-one-out: the aggregated TF-IDF description and mean rating are recomputed from the item's other training reviews only, and cluster membership is reassigned to the nearest centroid under the corrected profile), correcting for the profile otherwise partly reflecting the review whose rating is being predicted. Test-split rows are unaffected by this correction, since no test-split review ever contributes to any item's profile in the first place.
...
We replace the global sentiment stream with an aspect-based sentiment analysis (ABSA) module that reuses the same fine-tuned BERT [23] classifier as the baseline — no additional model training is introduced — but restructures how it is applied and how its output is fed to fusion (Fig. 1). For each review, sentences are matched against a domain-specific keyword lexicon (Table II); a review with no matching sentence falls back to a whole-review score, so every review contributes at least one signal.

**Notation.** Let $\mathcal{A} = \{a_1, \ldots, a_K\}$ be a domain's fixed aspect set ($K \in \{4,5,6\}$, Table II), and $A(r) \subseteq \mathcal{A}$ the aspects matched in review $r$. For $a \in A(r)$, let $\hat{s}(a,r) \in [0,1]$ be the BERT score on the sentences matched to $a$, and $n(a,r)$ the number of such sentences. Let $s_0(r) \in [0,1]$ be the whole-review fallback score, used whenever an aspect (or the whole review) has no match. A per-aspect confidence combines a score-margin term with an evidence-count term:

$$c(a,r) = \max\left(\frac{\overbrace{|2\hat{s}(a,r) - 1|}^{\text{margin}} + \overbrace{\min\!\left(n(a,r)/3,\ 1\right)}^{\text{evidence}}}{2},\ 0.05\right) \tag{1}$$

the margin term largest for decisive (near-0/1) scores, the evidence term growing with matched-sentence count (capped at 3), and the 0.05 floor preventing any aspect from being discarded outright. This confidence measure is a lightweight heuristic specific to this pipeline rather than one adapted from prior ABSA work.
...
The hotel taxonomy is taken directly from TripAdvisor's own sub-rating categories rather than authored generically, which — as discussed in Section V — corresponds to the highest keyword-match coverage of the three domains. We evaluate four strategies for converting this per-aspect representation into a fusion-stage feature, holding fusion itself (Section III-B) fixed across all four so that any RMSE difference is attributable to the sentiment representation alone. The first two aggregate matched aspects into a single scalar, matching the baseline's global-sentiment feature format; the latter two instead preserve the full per-aspect vector, consistent with the feature-combination design already used elsewhere in this architecture [16], [17].

1. **Mean.** Matched aspect scores are averaged, unweighted:
...
2. **Confidence-weighted mean.** As Eq. 2, but weighted by confidence $c(a,r)$ (Eq. 1), testing whether confidence-aware aggregation recovers the information a naive mean discards:
...
3. **Concat.** A value is retained for every aspect in $\mathcal{A}$, matched or not (unmatched aspects take the fallback score), passed to fusion as a raw, un-aggregated vector:
...
yielding $K$ raw features (4–6 depending on domain) in place of the single global-sentiment feature.

4. **Concat + confidence.** As Concat, but each aspect's confidence is appended as an explicit additional feature rather than used as an aggregation weight, doubling the feature count to $2K$. The confidence term reuses Eq. 1 with $n(a,r)=0$ substituted for $a \notin A(r)$, so the filled score $\tilde{s}(a,r)$ enters the margin term for unmatched aspects:
...
Unlike Eq. 1, $\tilde{c}(a,r)$ is not floored at 0.05 in the implementation used to produce the results reported below — an inconsistency between the two confidence formulas rather than a deliberate design choice, reported here for transparency. Concat + confidence is the only variant in which confidence acts as a signal to the regressor rather than an aggregation weight.

All five configurations (baseline plus four ABSA variants) share identical DeepMF and content-based clustering streams (Section III-B), neither of which depends on the sentiment representation; only the sentiment stream and the fusion-stage regressor differ across configurations.

### D. Experimental Setup

All five configurations (baseline plus four ABSA variants) and the two classical CF baselines are evaluated on each of the three domains under 5 random seeds (42, 123, 456, 789, 1011). The train/validation/test split and the fine-tuned BERT sentiment checkpoint are held fixed across seeds within a domain — seed variation affects only the stochastic components downstream of sentiment scoring (DeepMF weight initialization and training trajectory, K-means/agglomerative clustering initialization where applicable, and the DecisionTreeRegressor fit) — both a standard multi-seed evaluation practice for isolating model-variance from data-variance, and a practical necessity given the computational cost of repeated BERT fine-tuning.

### E. Evaluation Protocol

Root Mean Squared Error (RMSE) and Mean Absolute Error (MAE) on held-out test-set ratings are the evaluation metrics.

Statistical significance between the baseline and each ABSA variant is assessed with the Wilcoxon signed-rank test on paired per-sample squared errors, computed independently for each of the 5 seeds against the same test set. We report both the count of seeds reaching p < 0.05 and a Fisher-combined p-value across the 5 per-seed tests. Fisher's method assumes independent tests; since the 5 per-seed tests here share an identical test set (only the trained model differs by seed), the combined p-value is reported as a conventional cross-seed summary rather than as satisfying strict independence.
...
Before evaluating the sentiment-fusion question, we confirm that the reimplemented hybrid baseline substantially outperforms non-hybrid classical CF on all three domains, establishing that the architecture is a meaningful reference point rather than a strawman. Fig. 2 shows RMSE for item-KNN, SVD, and the hybrid baseline. The hybrid baseline reduces RMSE by 29–46% relative to item-KNN and by 27–42% relative to SVD, consistently across all three domains (restaurant: 0.693 vs. 1.202/1.075; e-commerce: 0.666 vs. 1.224/1.142; hotel: 0.650 vs. 0.916/0.895; all seed-level differences significant, 5/5 seeds, p < 0.001 by Fisher-combined test). This gap is consistent with the sparsity of all three domains (99.6–99.9%, Table I), where the content-based and sentiment streams provide signal that pure collaborative filtering cannot recover from interaction data alone.

![Fig. 2. Adapted hybrid baseline vs. non-hybrid classical CF (mean ± SD over 5 seeds)](figures/fig1_hybrid_vs_classical_cf.png)

### B. Effect of ABSA Sentiment-Fusion Strategy

Fig. 3 and Table III report RMSE for the baseline and all four ABSA variants across the three domains. The pattern is consistent in direction and approximate magnitude across all three domains despite their substantially different rating distributions, sparsity, and aspect-keyword coverage.
...
![Fig. 4. Raw per-seed RMSE (n=5 seeds each; horizontal bar = mean), baseline vs. Concat vs. Concat + confidence — the ringed point marks the seed with the largest |z-score| in the TripAdvisor hotel baseline](figures/fig3_variance_dotplot.png)
...
*[Reference [7] (Elahi et al.) has been verified and completed against `manuscript/SLR_SARS_REFERENSI_ALL.csv` (title, journal, DOI confirmed as "Hybrid recommendation by incorporating the sentiment of product reviews," Information Sciences, 2023) — however, the CSV's abstract field is too generic to confirm the specific methodology attributed to it in Section II (BERT sentence embeddings + item-based CF/ALS/DeepFM); that description should be re-checked against the full paper before submission. References [12], [13], [15]–[17] still have incomplete author/title metadata (no match found in the SLR CSV) and must be verified against the original papers before submission — flagged rather than fabricated. Note also that [18]–[22] are now cited earlier in the text (Introduction) than [7]–[17] (Related Work) — strict IEEE numbering requires reference numbers to follow first-citation order, so a final renumbering pass (e.g., via a reference manager) is needed before submission; nothing about the reference CONTENT is affected, only the numbering.]*
